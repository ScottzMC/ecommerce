...
<head>
    ...
    <?php foreach($sort as $srt){} ?>
    <title>Sort <?php echo $srt->type; ?> Categories || UG Store</title>
        ...
        <div class="breadcrumb-area">
            <div class="container">
                <div class="breadcrumb-content">
                    <h2><?php echo $srt->type; ?> Categories</h2>
                    <ul>
                        <li><a href="<?php echo site_url('home'); ?>">Home</a></li>
                        <li class="active"><?php echo $srt->type; ?></li>
                    </ul>
                </div>
            </div>
        </div>
                            ...
                            <div class="kenne-sidebar_categories">
                                ...
                                <ul class="sidebar-checkbox_list">
                                  ...
                                  foreach($query as $qry){ ?>
                                    <li>
                                      <?php
                                      $sequel = $this->db->query("SELECT COUNT(brand) AS total_brand FROM product_details WHERE brand = '$qry->brand' ")->result();
                                      foreach($sequel as $sql){
                                      }
                                      ?>
                                      <?php if(!empty($sql->total_brand)){ ?>
                                        <a href="<?php echo site_url('other/brand_type/'.strtolower($srt->type).'/'.strtolower($qry->brand)); ?>">
                                          <?php echo $qry->brand; ?> (<?php echo $sql->total_brand; ?>)
                                        </a>
                                      <?php } ?>
                                    </li>
                                    <?php } ?>
                            ...
                            <div class="kenne-sidebar_categories">
                                ...
                                <ul class="sidebar-checkbox_list">
                                  ...
                                  foreach($query as $qry){ ?>
                                    <li>
                                      <?php
                                      $sequel = $this->db->query("SELECT COUNT(color) AS total_color FROM product_details WHERE color = '$qry->color' ")->result();
                                      foreach($sequel as $sql){
                                      }
                                      ?>
                                      <?php if(!empty($sql->total_color)){ ?>
                                      <a href="<?php echo site_url('other/color_type/'.strtolower($srt->type).'/'.strtolower($qry->color)); ?>">
                                        <?php echo $qry->color; ?> (<?php echo $sql->total_color; ?>)
                                      </a>
                                    <?php } ?>
                                    </li>
                                    <?php } ?>
                            ...
                            <div class="kenne-sidebar_categories">
                                ...
                                <ul class="sidebar-checkbox_list">
                                  ...
                                  foreach($query as $qry){ ?>
                                    <li>
                                      <?php
                                      $sequel = $this->db->query("SELECT COUNT(size) AS total_size FROM product_details WHERE size = '$qry->size' ")->result();
                                      foreach($sequel as $sql){
                                      }
                                      ?>
                                      <?php if(!empty($sql->total_size)){ ?>
                                        <a href="<?php echo site_url('other/size_type/'.strtolower($srt->type).'/'.strtolower($qry->size)); ?>">
                                          <?php echo $qry->size; ?> (<?php echo $sql->total_size; ?>)
                                        </a>
                                      <?php } ?>
                                    </li>
                                    <?php } ?>
                    ...
                    <div class="col-xl-9 col-lg-8 order-1 order-lg-2">
                        <div class="shop-toolbar">
                            ...
                            <div class="product-item-selection_area">
                              <form action="<?php echo base_url('sort/type/'.strtolower($srt->type)); ?>" method="post">
                                <div class="product-short">
                                    <label class="select-label">Short By:</label>
                                    <select class="nice-select myniceselect" name="order">
                        ...
                        <div class="shop-product-wrap grid gridview-3 row">
                          <!-- Loop -->
                          <?php if(!empty($sort)){ foreach($sort as $srt){ ?>
                            <div class="col-lg-4 col-md-4 col-sm-6">
                                <div class="product-item">
                                    <div class="single-product">
                                        <div class="product-img">
                                            <a href="<?php echo site_url('product/detail/'.strtolower($srt->product_code)); ?>">
                                                <img height="250" class="primary-img" src="<?php echo base_url('uploads/products/'.$srt->image); ?>" alt="<?php echo $srt->title; ?>">
                                            </a>
                                                ...
                                                <ul>
                                                  ...
                                                  if(!empty($email)){
                                                    ...
                                                    if($membership == 'Premium Member'){
                                                      $price = $srt->price - ($srt->price * 20/100);
                                                    }else if($membership == 'Elite Member'){
                                                      $price = $srt->price - ($srt->price * 30/100);
                                                  ...
                                                  <form action="<?php echo base_url('shopping/wishlist/add'); ?>" method="post">
                                                    <input type="hidden" name="id" value="<?php echo $srt->id; ?>">
                                                    <input type="hidden" name="email" value="<?php echo $this->session->userdata('uemail'); ?>">
                                                    <input type="hidden" name="product_code" value="<?php echo $srt->product_code; ?>">
                                                    <input type="hidden" name="title" value="<?php echo $srt->title; ?>">
                                                    <input type="hidden" name="type" value="<?php echo $srt->type; ?>">
                                                    <input type="hidden" name="category" value="<?php echo $srt->category; ?>">
                                                    <?php if(!empty($email) && !empty($membership) == 'Normal' || !empty($membership) == 'Premium Member' || !empty($membership) == 'Elite Member'){ ?>
                                                      <input type="hidden" name="price" value="<?php echo $price; ?>">
                                                    <?php }else{ ?>
                                                      <input type="hidden" name="price" value="<?php echo $srt->price; ?>">
                                                    <?php } ?>
                                                    <input type="hidden" name="image" value="<?php echo $srt->image; ?>">
                                                    <li>
                                        ...
                                        <div class="product-content">
                                            <div class="product-desc_info">
                                                <h3 class="product-name"><a href="<?php echo site_url('product/detail/'.strtolower($srt->product_code)); ?>"><?php echo $srt->title; ?></a></h3>
                                                <div class="price-box">
                                                  <?php if(!empty($email) && !empty($membership) == 'Normal' || !empty($membership) == 'Premium Member' || !empty($membership) == 'Elite Member'){ ?>
                                                    <span class="new-price">&pound;<?php echo $price; ?></span>
                                                  <?php }else{ ?>
                                                    <span class="new-price">&pound;<?php echo $srt->price; ?></span>
                                                  <?php } ?>
